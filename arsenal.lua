...
local OldNameCall
OldNameCall = hookmetamethod(game, "__newindex", function(A, B, self, ...)
    if not checkcaller()  and (B == "WalkSpeed" or B == "walkspeed")  then
        return 20
    end
    if not checkcaller()  and (B == "JumpPower" or B == "jumppower")  then
        return 15
    end
    return OldNameCall(A, B, self, ...)
end)
end 
    ...
    if setfflag then 
        Player.Character.Humanoid:ChangeState(11)
            ...
            for i, v in pairs(Players:GetPlayers()) do
                if
                    Player.Character ~= nil and v ~= Player and
                        (v.TeamColor ~= Player.TeamColor or v.TeamColor == "Bright blue") and
                        v.Character ~= nil and
                        v.Character:FindFirstChild("Gun")
                 then
                    if
                        v ~= Player and v:FindFirstChild("NRPBS") and v.NRPBS.Health.Value > 0 and
                            v.Character and
